...
class ScrabbleTest < MiniTest::Unit::TestCase
  ...
  def test_is_valid_word
    ...
    word = "baz"
    assert !g.is_valid_word(word, hand)
  end

  def test_is_valid_word_duplicate
    g= Scrabble::Game.new
    hand = {'o' => 1, "b" => 1, "z" => 1, "t" => 1, "s" => 2 }
    word = "boots"
    assert !g.is_valid_word(word, hand)
    hand = {"o" => 2 , "b" => 1, "t" => 1, "z" => 1 }
    assert g.is_valid_word(word, hand)
  end

  def test_initialize_word_hash
    g = Scrabble::Game.new
    assert g.words_list.keys.include?(g.words.sample)
  end
